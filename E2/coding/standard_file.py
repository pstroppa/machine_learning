...
filename = " .csv"
methode = "  "
energy_df = pd.read_csv("E2/data/" + filename, sep=";",
...
#  evaluation for all 5 measures (see pptx numeric_values: Slide 36):
    ...
    rrse1 = sqrt(sum(np.multiply(y1-y_test, y1-y_test)) /
                 sum(np.multiply(y_test-mlist, y_test-mlist)))
    ...
    rrse2 = sqrt(sum(np.multiply(y2-y_test, y2-y_test)) /
                 sum(np.multiply(y_test-mlist, y_test-mlist)))
    ...
    rrse3 = sqrt(sum(np.multiply(y3-y_test, y3-y_test)) /
                 sum(np.multiply(y_test-mlist, y_test-mlist)))
    ...
    rrse4 = sqrt(sum(np.multiply(y4-y_test, y4-y_test)) /
                 sum(np.multiply(y_test-mlist, y_test-mlist)))
    ...
    win = min(entry)
    lose = max(entry)
    print("rmse: "+"{:7.3f}".format(win),
          "   Preprocessing winner: ", entry.index(win)+1)
    ...
    #print(entry)
    win2 = min(entry2)
    print("rrse: "+"{:7.3f}".format(win2),
          "   Preprocessing winner: ", entry2.index(win2)+1)

    entry3 = [mae1, mae2, mae3, mae4]
    #print(entry)
    win3 = min(entry3)
    lose3 = max(entry3)
    print("mae: "+"{:8.3f}".format(win3),
          "   Preprocessing winner: ", entry3.index(win3)+1)
    ...
    #print(entry)
    win4 = min(entry4)
    print("rae: "+"{:8.3f}".format(win4),
          "   Preprocessing winner: ", entry4.index(win4)+1)

    entry5 = [cor1, cor2,  cor3, cor4]
    #print(entry)
    win5 = max(entry5)
    lose5 = min(entry5)
    print("cor: "+"{:8.3f}".format(win5),
          "   Preprocessing winner: ", entry5.index(win5)+1)
