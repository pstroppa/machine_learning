...
#best result: tree, prepro X4, parameter 2, 12
#missing values: -200 ergibt bessere ergebnisse als mean
...
air_df = pd.read_csv("Data/"+filename, sep =";", lineterminator="\n", encoding="utf-8",error_bad_lines=False)

######################################################################
#general preprocessing (not associated with any methode or any of the four preprocessing methods later on)
air_df = air_df[:9357]           

air_df=air_df.drop(columns=['NMHC(GT)','CO(GT)','NOx(GT)','NO2(GT)','Unnamed: 15','\r'])
...
#applying methodes:
    ...
    if type(methode) is str:
        if methode is "lasso":
            #best pre X1, best parameters: alpha=0.005, a little worse than linear
            #but more stable
            methode = linear_model.Lasso(alpha=0.005)    
        elif methode is "knn":
            #best pre X1, best parameters: k= 5, weights='distance'
            k=5
            weigh ="distance"
            methode=neighbors.KNeighborsRegressor(k, weights=weigh)
            y_test=np.array(y_test)  
        elif methode is "tree":
            #best pre: X4, best parameters: 2, 12, best trade-off between being stable 
            #and being fine-grained enough
            methode = tree.DecisionTreeRegressor(min_samples_leaf=2, max_depth=12)   
        elif methode is"linear":
            #best_pre: X1,best parameter: fit_intercept=True  
